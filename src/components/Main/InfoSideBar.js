...
export function InfoSideBar({
  ...
  var today = new Date();
  console.log(parseInt(String(today.getMonth() + 1).padStart(2, '0')))
  return (
    <div
      className={` h-full col-span-2 noscb overflow-y-auto bg-bgDark rounded-2xl relative ${
        showInfo ? "right-0 w-2/5" : "right-[-100%] w-0"
    ...
    >
      <div className="w-full px-4 py-4 pt-0 flex flex-col gap-4 ">
        <div className="w-full flex items-center px-2 justify-between sticky top-0 left-0 z-[4] py-4 pb-2 bg-gradient-to-b from-bgDark via-bgDark to-bgdark/60 ">
          ...
          <div
            className="p-1 text-white/40 text-3xl"
            onClick={() => {
              setshowInfo(false);
            }}
          >
                  ...
                  {data?.results?.seasons?.map(
                    (e, i) =>
                      (parseInt(e?.air_date.substring(0, 4)) <=
                        ...
                        >
                          <SelectItem value={e?.season_number.toString()}>
                            {e?.name}
                          </SelectItem>
                        </Link>
                      )
